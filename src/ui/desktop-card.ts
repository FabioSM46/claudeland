...
import { formatPercent } from '../domain/usage.js';
...
export class DesktopCard {
  ...
  constructor(
    ...
    this.title = new St.Label({
      text: 'CLAUDE · CAPACITÀ RIMANENTE',
    ...
    if (!layoutManager._backgroundGroup) {
      throw new Error('Il gruppo background di GNOME Shell non è disponibile');
    }
  ...
  private render(state: Readonly<UsageState>): void {
    ...
    this.title.text = state.stale
      ? 'CLAUDE · DATI NON AGGIORNATI'
      : 'CLAUDE · CAPACITÀ RIMANENTE';

    if (!state.snapshot) {
      this.content.add_child(new St.Label({
        text: state.loading ? 'Caricamento…' : (state.error ?? 'Dati non disponibili'),
